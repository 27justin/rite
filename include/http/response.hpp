...
struct http_response {
    ...
    void trigger(enum event ev) {
        if(events_.contains(ev)) events_[ev](*this);
    }
    ...
    size_t content_length() {
        if(!headers_.contains("Content-Length")) return 0;
    ...
    void stream(std::string_view str) {
        stream(std::span<std::byte>((std::byte *) str.data(), str.size()));
    }

    void stream(std::vector<std::byte> &&data) {
